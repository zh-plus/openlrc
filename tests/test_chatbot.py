...
@pytest.fixture
def gpt_bot():
    return GPTBot(model='gpt-3.5-turbo-16k', temperature=1, top_p=1, retry=8, max_async=16, fee_limit=0.05)


def test_estimate_fee(gpt_bot):
    messages = [
        {'role': 'system', 'content': 'You are gpt.'},
        {'role': 'user', 'content': 'Hello'},
    ]
    fee = gpt_bot.estimate_fee(messages)
    assert isclose(fee, 2.6e-05)


def test_update_fee(gpt_bot):
    Response = namedtuple('response', 'usage')
    gpt_bot.api_fees += [0]
    response1 = Response({'prompt_tokens': 100, 'completion_tokens': 200, 'total_tokens': 300})
    gpt_bot.update_fee(response1)

    gpt_bot.api_fees += [0]
    response2 = Response({'prompt_tokens': 200, 'completion_tokens': 400, 'total_tokens': 600})
    gpt_bot.update_fee(response2)

    gpt_bot.api_fees += [0]
    response3 = Response({'prompt_tokens': 300, 'completion_tokens': 600, 'total_tokens': 900})
    gpt_bot.update_fee(response3)

    assert gpt_bot.api_fees == [0.0011, 0.0022, 0.0033]


def test_message(gpt_bot):
    messages_list = [
        [
    ...
    results = gpt_bot.message(messages_list)
